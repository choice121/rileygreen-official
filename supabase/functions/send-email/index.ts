...
async function smtpSend(to: string, subject: string, html: string): Promise<void> {
  ...
  try {
    ...
    const ehlo = await cmd(`EHLO mx.morganwallen.fan`)
...
function tplNewsletterConfirm(name: string) {
  ...
  return {
    ...
    html: wrap(
      ...
      p(`Hey ${first},`) +
      p("Thanks for subscribing to the Morgan Wallen newsletter. You'll be the first to hear about new music, tour announcements, merch drops, and everything going on.") +
...
function tplWelcome(name: string) {
  ...
  return {
    ...
    html: wrap(
      ...
      p(`Hey ${first},`) +
      p("Your Morgan Wallen fan account has been created. You now have access to early announcements, exclusive content, and more coming soon.") +
